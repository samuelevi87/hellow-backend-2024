...
public class JogoDecidaOCaminho {
    ...
    private static void iniciarAventura() {
        System.out.println("Você está na entrada de uma floresta assombrada com seus amigos. Deseja entrar? (sim/não)");
        String escolha = scanner.nextLine().toLowerCase();

        if (escolha.equals("sim")) {
            dentroDaFloresta();
        } else {
            System.out.println("Você decidiu não entrar na floresta porque é medroso. Fim da aventura!");
        }
    }

    private static void dentroDaFloresta() {
        System.out.println("Você entrou na floresta e se depara com um caminho dividido. Você vai para o mata alta ou barro molhado? (mata/barro)");
        String escolha = scanner.nextLine().toLowerCase();

        if (escolha.equals("mata")) {
            caminhoEsquerda();
        } else {
            caminhoDireita();
        }
    ...
    private static void caminhoEsquerda() {
        System.out.println("Você segue pela esquerda e encontra um esgoto cheio de bixos. Deseja nadar ou continuar caminhando? (nadar/continuar)");
        String escolha = scanner.nextLine().toLowerCase();
