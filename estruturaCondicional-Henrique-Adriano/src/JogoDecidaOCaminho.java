...
public class JogoDecidaOCaminho {
    ...
    private static void dentroDaFloresta() {
        System.out.println("Você entrou na floresta e se depara com um caminho dividido. Você vai para a esquerda ou direita? (esquerda/direita/centro)");
        String escolha = scanner.nextLine().toLowerCase();

        if (escolha.equals("esquerda")) {
            caminhoEsquerda();
        } else if (escolha.equals("direita")){
            caminhoDireita();
        } else {
            System.out.println("Você se perdeu e voltou ao início");
            dentroDaFloresta();
        }
    }
